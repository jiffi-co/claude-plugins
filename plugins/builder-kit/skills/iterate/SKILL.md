...
- Use when a product is already built and shipped and the user wants to add or change a feature: "add X", "let's extend Y", "/iterate".
...
- docs/, ADRs, the acceptance checklist and your phase tracker (Beads if you use it, otherwise native Tasks or `docs/tasks.md`) are the source of truth. Ground in the files, not in chat memory, and never overstate what the live code already does.
...
- `docs/implementation-plan.md`: appended feature phases. `docs/checkpoints/phase-<N>.json` and one issue per new phase (Beads if you use it, otherwise native Tasks or a `docs/tasks.md` checklist).
