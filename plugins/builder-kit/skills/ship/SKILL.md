---
name: ship
description: Take a green, committed phase live. Runs the bundled /code-review and /security-review, opens the PR via gh, holds for HUMAN sign-off, merges, deploys per project type to the hosting ADR's target (Vercel is one web option, TestFlight for iOS, the agent host for agent), applies the production database migration on a web deploy, then smoke-checks the release. Fires when the phase checkpoint is green, the working tree is clean, and the branch is pushed.
...
2. **Run the bundled reviews.** Both ship with Claude Code — no plugin to install:
   ...
   Then route to the fresh-context reviewer agents (via the Agent tool, not slash commands): the `security-auditor` agent for a deeper, project-aware security pass on every project type, plus the reviewer keyed to `projectType`. For a **web** build, the `seo-specialist` agent checks the crawlable pages against the SEO checklist. For an **agent** build, the `agent-eval` agent runs straight after `security-auditor`. The security pass reads the diff statically; `agent-eval` goes further and actually RUNS the eval scenarios and adversarial probes (prompt injection, jailbreak, loop and cost caps), judging how the agent behaves when pushed. Treat it as a hard gate: a single clean injection or jailbreak bypass, an uncapped loop, or a leaked secret is a FAIL that blocks the ship, so fix it and re-run until it returns PASS. (An **ios** build's extra gate, `ios-release-checklist`, runs later in the ios branch, right before the archive.) Fix what every reviewer raises and re-run until clean. These agent reviews sit alongside the bundled reviews above; none of them replaces the human code review at step 5.

   **Consult the concerns checklist before shipping.** Read `.claude/rules/concerns.md` (the six persistent rubrics scaffolded by `/jiffi-init`: accessibility, security, performance, seo, legal, i18n) and walk every rule that applies to this release's surface. The bundled reviews and reviewer agents cover security and (for web) seo, but accessibility, performance, legal and i18n can fall through the gaps, so name each applicable rule and confirm it is addressed with evidence (a passing check, a captured artefact, the `verify-acs` ledger row), not assumed. Carry any unresolved concern into the human code review at step 5 as an explicit item; a rule that genuinely does not apply is noted N/A with the reason. Do not treat a clean `/security-review` as clearing the whole checklist.
...
- **Consult the concerns checklist before shipping.** Walk `.claude/rules/concerns.md` for every rule that applies to the release's surface (accessibility, security, performance, seo, legal, i18n) and confirm each is addressed with evidence; surface any unresolved concern at the human gate rather than shipping past it.
