---
name: create-adr
description: Write one Architecture Decision Record per significant technical choice (framework, database, ORM, styling, auth, hosting) and per cross-cutting decision a non-developer would miss (security boundary, environment separation, observability, cost and abuse control, data migration and backup, connection pooling, accessibility target), each with alternatives and consequences, plus the ADR index. Fires when architect has confirmed decisions, or when docs/prd/prd.md exists and docs/adr/ does not yet cover every decided area.
...
- **Data migration, backup and rollback.** A migration tool, migrations reviewed and reversible (whether the tool generates them from a schema, as Drizzle or Prisma do and you diff to prove the schema is current, or you hand-author them in its API, as Kysely or Knex do and that is the source of truth, both are valid; only ad-hoc DDL run outside the migration system is a fail), no destructive change without a backup first, and point-in-time recovery confirmed on with a known retention window.
- **Connection pooling.** This follows how the runtime reaches the database, not which SQL engine is chosen. A long-lived server wants a bounded connection pool fronted by a pooler (PgBouncer for Postgres, ProxySQL for MySQL, or an equivalent). Serverless or edge invocations each open their own connection and hit the limit under load, so reach the database over an HTTP or serverless driver instead (`@neondatabase/serverless` for Neon Postgres, `@planetscale/database` for PlanetScale MySQL, or Cloudflare Hyperdrive fronting a pool). Note that HTTP or serverless drivers cannot hold an interactive `SELECT ... FOR UPDATE` row-lock transaction on any engine, so a workload that needs one wants the pooled-server shape. Decide now and record it in the database or hosting ADR.
