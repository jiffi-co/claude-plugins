---
name: prd
description: Turn an approved Idea Pack into a PRD with numbered, testable acceptance criteria, a data model, contracts and a phased delivery plan, asking the four judgement questions (sensitive data, regulated data, the accessibility level, the running-cost ceiling) rather than assuming them. Sections follow projectType in .claude/builder-kit.json (web carries SEO and UI/UX, ios adds privacy and data collection plus on-device persistence and accessibility, agent adds a tool allowlist plus eval scenarios and a cost budget). Fires when docs/idea/idea-pack.md is approved and docs/prd/prd.md does not exist.
allowed-tools: [Read, Write, AskUserQuestion, Skill, mcp__context7__resolve-library-id, mcp__context7__query-docs]
---
...
3. **Ask the four judgement questions.** These are the ones a first-time builder does not know to raise, and they change what the PRD has to contain, so the kit asks them rather than leaving them on a page for someone to remember. One `AskUserQuestion` card each, plain-language headline, two or three options with a default flagged, and one sentence on what turns on it.
   1. **What personal or sensitive information does version one touch?** Names, email addresses, messages, location, payment details, health or financial data, anything about children, or none of the above. The answer sets the data model's sensitivity notes, the non-functional security bar, and (on iOS) the privacy and data-collection section that the App Store answers must match.
   2. **Does it touch regulated information?** Health records, financial accounts, government identifiers, or data about people in a jurisdiction with its own rules. If yes, name the regime in the PRD and record that the compliance detail is a decision for the human, not something this skill resolves. If no, write that down too, because "we checked and it does not" is worth having on the record.
   3. **What accessibility level are we building to?** WCAG AA is the house default and the answer most builders should take. This lands in the non-functional bar and flows through to the design system, and it is a real exposure when it is skipped rather than declined.
   4. **What is the running-cost ceiling?** Read this back rather than asking again: show it and confirm it still holds. Two places carry it, in this order — `costCeiling` in `.claude/builder-kit.json` (written by `/builder-kit:start` at the front door), then the `Monthly cost ceiling:` line in section 10 of `docs/idea/idea-pack.md`. If neither has it, ask it here as a fresh question and say that it should have been recorded earlier, rather than assuming a number nobody named. It becomes the cost line in the non-functional bar, and the architecture step reads it back a second time as its fifth decision.

   Record all four answers in the PRD where they belong (the data model, the non-functional bar, out of scope), not as a separate appendix. If a question genuinely does not apply (an offline tool that stores nothing about anyone), record that conclusion and why, rather than dropping the question.
4. **Ground the load-bearing versions, do not recall.** Context7's budget is limited (plan on roughly three `resolve-library-id` calls), so spend it where a wrong version is expensive: confirm the load-bearing, security-relevant dependencies first (auth, the data layer, payments) with Context7 (`resolve-library-id`, then `query-docs`), not the frameworks everyone already knows. builder-kit bundles Context7 for exactly this. For the rest, name the dependency and flag its version to confirm at the architecture/ADR step, where `architect` grounds every candidate library against Context7 before the ADRs are accepted. If no stack is chosen yet (the PRD legitimately comes before architecture), do not force a framework or version now: name the load-bearing dependency by its role (the auth provider, the data layer, the payment processor) and defer its version grounding to the architecture step, where `architect` picks the actual library and grounds it. A named role with a deferred version is honest; a guessed framework is not. Do not leave a silent guess. If Context7 is unavailable or cannot confirm something, say so in the PRD rather than guessing a version.
5. **Derive numbered, testable acceptance criteria.** Number them `AC-<US>.<n>` (e.g. AC-001.1). Each is a single statement checkable by a test, a build check, or a request you can make and assert on, not a matter of opinion. Every user story gets at least one happy-path AC and at least one edge-case or error AC. For an agent, at least one of those edge cases is adversarial: a prompt-injection attempt, a tool error, or a request it must refuse.
6. **Propose a phased delivery plan** where Phase 1 is the smallest shippable slice, and note which phases can run in parallel and which must run in order.
7. **Show the user and revise before finalising**, then write the PRD and the paired acceptance checklist.
...
9. **Dependencies.** External services, libraries and SDKs. Ground the load-bearing versions in Context7 and flag the rest to confirm at the architecture step (Process step 4). Note what breaks if one is unavailable.
...
- **The four judgement questions are asked, never assumed** (Process step 3): what personal or sensitive information version one touches, whether it touches regulated information, the accessibility level, and the running-cost ceiling read back from the Idea Pack. Record every answer in the PRD, including "none", because a question that was put and declined is evidence while a question that was never put is a gap.
