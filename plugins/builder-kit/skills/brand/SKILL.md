...
Every one of the five choices below is shown, never described. For each step write a self-contained HTML file into `docs/brand/` (inline CSS only, zero external dependencies, no CDN, no web fonts unless embedded), tell the builder the exact path to open in a browser, and let them choose by looking. Never ask them to imagine a look or pick from a hex list or prose. Any HTML that shows a colour pairing must display the pairing's computed WCAG contrast ratio and its AA verdict, calculated in an inline `<script>` from the relative-luminance formula (so the number is real, not eyeballed), since this skill has no Bash to compute it out of band.
