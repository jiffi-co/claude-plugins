...
Every one of the five choices below is shown, never described. For each step write a self-contained HTML file into `docs/brand/` (inline CSS only, zero external dependencies, no CDN, no web fonts unless embedded), tell the builder the exact path to open in a browser, and let them choose by looking. Never ask them to imagine a look or pick from a hex list or prose. Any HTML that shows a colour pairing must display the pairing's computed WCAG contrast ratio and its AA verdict, calculated in an inline `<script>` from the relative-luminance formula (so the number is real, not eyeballed), since this skill has no Bash to compute it out of band. A candidate colour must pass AA before it ships as a candidate.
...
3. **Palette.** Write `docs/brand/palette.html`: two or three candidate palettes rendered as swatches (primary, secondary, accent, neutrals, surface, text), each pairing shown with its computed WCAG contrast ratio and AA verdict via the inline script. Bias candidates toward the tone chosen in step 2. Tell them to open it, capture the pick with `AskUserQuestion`, and record the chosen palette in `docs/brand/palette.md`.
...
- Every colour pairing carries its computed WCAG contrast ratio and AA verdict; a candidate colour passes AA before it ships as a candidate. If a brand colour the human insists on cannot reach AA, flag it and recommend an accessible adjustment rather than silently restyling their brand.
